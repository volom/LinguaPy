"""
Exercise for translation word from russian to English
"""
import re
import random
import pyttsx3
...
from db_repo.db_handle import get_specific_word_rus
from db_repo.db_handle import get_specific_word

def t_word_rus2eng(word):
    word = get_specific_word('w_translated', 'word_rus', word)[0][0]
    while True:
        word_init = word
        ...
        all_words_eng = [x[0] for x in get_columns_info_db('w_translated', ('word_eng'))]
        right_translation = get_specific_word_rus('w_translated', 'word_eng', word)[0][0]
        all_words_eng.remove(right_translation)
        banch_words = random.sample(all_words_eng, 4)
